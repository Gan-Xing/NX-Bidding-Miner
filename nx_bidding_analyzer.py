...
# --- 关键修复：解决打包后 Playwright 找不到浏览器的问题 ---
# 设置环境变量，强制 Playwright 使用系统全局的浏览器路径
...
class BiddingApp:
    def __init__(self, root):
        self.root = root
        self.root.title("宁夏公共资源交易分析工具 v3.5.1 (打包优化版)")
...
"""
    ...
    def setup_ui(self):
        ...
        # 按钮文本更新，提醒用户必要性
        ttk.Button(top_frame, text="🛠️ 初次运行点击：一键安装/修复环境", command=self.install_env).pack(side=tk.LEFT)
    ...
    def install_env(self):
        def run():
            try:
                self.log("正在为您准备运行所需的浏览器组件 (大小约100MB)...")
                # 如果是打包环境，subprocess调用需要考虑运行时的环境路径
                ...
                if process.returncode == 0:
                    self.log("✅ 环境准备就绪！现在可以点击启动任务了。")
                    self.root.after(0, lambda: messagebox.showinfo("成功", "环境已就绪！"))
                else:
                    self.log("❌ 安装过程出现异常。")
    ...
    def start_task(self):
        ...
        if not key: return messagebox.showerror("错误", "API Key 缺失")
    ...
    async def crawl_all_pages(self, keyword):
        ...
        async with async_playwright() as p:
            try:
                ...
                self.log(f"❌ 浏览器启动失败！请先点击左上角【一键安装环境】再运行。\n详情: {e}")
            ...
            async def handle_response(response):
                if "getFullTextDataNew" in response.url:
                    try:
                        ...
                        if recs:
                            self.root.after(0, lambda: self.log(f"📥 成功捕获本页 {len(recs)} 条数据..."))
                            for r in recs:
                                raw_db.append({
                                    "id": len(raw_db),
        ...
        if not raw_db:
            ...
            self.log(f"✅ 抓取完成共 {len(raw_db)} 条。进入 AI 分比提取分析...")
            all_final = []
            for i in range(0, len(raw_db), 5):
                batch = raw_db[i:i+5]
                self.log(f"分析第 {i//5 + 1} 批数据...")
    ...
    def analyze_batch_json(self, batch, keyword):
        try:
            prompt = f"分析以下公告，识别中标单位和金额。返回 JSON 包含 ID。关键词: {keyword}\n\n"
            for d in batch:
                prompt += f"--- [ID: {d['id']}] ---\n内容: {d['content'][:2500]}\n\n"
        ...
        for i in range(0, len(db), 5):
            ...
            for item in res:
                ...
                if rid is not None and rid < len(db):
                    orig = db[rid]
                    finals.append({"项目标题": orig["title"], "中标单位": item.get("winner"), "中标金额": item.get("amount"), "发布日期": orig["date"], "源链接": ""})
        self.save_to_excel(finals)

    def save_to_excel(self, results):
        if not results: return self.log("⚠️ 无有效数据。")
